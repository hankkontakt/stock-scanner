...
def _apply_sanity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - pe_trailing/pe_forward: icke-finit/<=1/>200 -> NA; dessutom pe < 6 -> NA
      ...
      APP 3.68, CME 3.66, LIN 5.18, LLY 5.59; äkta pe < 6 är ovanligt och fel
      framställs som "billig" för mega-bolag)
    ...
    if "dividend_yield" in df.columns:
        ...
        frac = v.copy()
        # ROND 6: rate-baserad beräkning (enhetsfri, matchar data_fetcher._sanity_check)
        # vinner om dividend_rate + current_price finns — yfinance levererar BÅDE %
        # (0.44 = 0.44 %) och fraktion (0.0044) inkonsekvent, men rate/price är alltid rätt.
        if "dividend_rate" in df.columns and (
            ...
            # Fallback-varning-logg: %-värden i (0.1, 1] från info för de utan rate
        frac.loc[v > 1] = v.loc[v > 1] / 100
        # Endast om rate saknas: tolka %-värden 0.1-1 som % (0.44 = 0.44%)
